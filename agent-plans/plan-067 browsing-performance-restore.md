...
| 5 | MOSTLY DONE (plan-069) — **Light-hash**. Layers 1–4 done: sampler primitive + `blake3_light` column/`NewFile` + Full/Light `HashPolicy` (Light = sampled-only, never reads whole file) + `scans.hash_policy` + **light scans excluded from exact dedup/delete-check (safety)** + scan-start Full/Light modal (via plan-070 #4). REMAINING (069 layers 5–7): `likely_safe` "Likely covered elsewhere" delete-check signal (SAFETY-CRITICAL), CAS metadata store, `blake3_light` grid column, CLI `--hash-policy` flag. | 064 | scanner.rs, db.rs | P2 |
...
| 7 | PARTIAL — **Discovery benchmark** done (`scanner::benchmark_discovery` + CLI + test). `NewFile.blake3_light` blocker now RESOLVED (landed with #5 layer 2). `perf_gates.rs` still needs `ScanRunnerKind`/`run_prepared_scan_with_runner` (a 2nd tokio runner — test-only) + `Database::delete_check_scoped` to compile the gate suite. | 036,038,065 | scanner.rs; tests/perf_gates.rs | P2 |
