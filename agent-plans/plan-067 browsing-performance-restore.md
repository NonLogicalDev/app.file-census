...
| 5 | PARTIAL (plan-069) — **Light-hash**. Step 1 done: `blake3_light` sampler primitive (compile-time knobs, terminal+interior slices, whole-file threshold ⇒ equals full BLAKE3, samples large files) + 3 tests. DEFERRED as own layers: schema column + population, Full/Light policy, the `likely_safe` delete-check taxonomy (SAFETY-CRITICAL), CAS metadata store, CLI/UI. Was `in-progress`/never-finished even pre-deletion. | 064 | scanner.rs | P2 |
| 6 | ✅ DONE (core) — **EXIF enrichment**. Found `migrate()` never created the `file_exif` table (fresh DBs lacked it) — added it + index. `file_details` now caches every extraction by (blake3,size) and reads the cache when the drive is offline (EXIF survives disconnects). `file-extra-info exif <scan>` runs the enrichment pass with terminal statuses. Cache round-trip test + E2E. NOTE: `app_events`/`app_settings`/`file_extra_info_runs` are also missing from migrate but unreferenced in recovered code (persistent event journal not recovered — separate item). Deferred: async task + `file_extra_info.stop` RPC (the pass is synchronous). | 040 | db.rs migrate; media.rs | P2 |
| 7 | PARTIAL — **Discovery benchmark** restored: `scanner::benchmark_discovery` (+ `DiscoveryStats`) + `scans benchmark-discovery` CLI (entries/files/dirs, first-*-latency, throughput, `--stop-after-ms` probe) + unit test. E2E verified. STILL BLOCKED: `perf_gates.rs` also needs `ScanRunnerKind`/`run_prepared_scan_with_runner` (a 2nd tokio runner — test-only), `delete_check_scoped`, and `NewFile.blake3_light` (from #5), so the gate suite can't compile until #5 lands. | 036,038,065 | scanner.rs; cli.rs | P2 |
