...
4. [ ] Recover a strict composite temporary tree using verified source snapshots as file-local horizons around non-patch formatter transitions; promote only validated, coherent source into this checkout.
...
- A direct snapshot-index pass found 105 complete pre-deletion file reads. The recovery tool currently recognizes 27 of the highest-value snapshots and reproduces a 25-file, 244 KB exact-source tree containing Rust backend modules, React shell/pages/components, build configuration, and the redesign location screen. It is intentionally labeled as a source snapshot collection rather than a buildable application because the snapshot timestamps differ.
...
- [ ] Add the complete verified snapshot index to the recovery tool and validate strict file-local horizon replay before copying any recovered source into the checkout.
