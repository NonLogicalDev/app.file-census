fn main() {
    let light_hash_slice_count = env_usize("FILE_CENSUS_LIGHT_HASH_SLICE_COUNT", 2);
    if light_hash_slice_count < 2 {
        panic!("FILE_CENSUS_LIGHT_HASH_SLICE_COUNT must be at least 2");
    }
    let light_hash_slice_width = env_u64("FILE_CENSUS_LIGHT_HASH_SLICE_WIDTH", 5 * 1024 * 1024);
